...
test_8.1 <- function(){
    properties <- c(choosing_k$mapping, choosing_k$layers[[1]]$mapping)
    ...
    test_that('k should be on the x-axis.', {
        expect_true("k" %in% c(rlang::get_expr(properties$x),rlang::get_expr(properties$x)))
    })
    test_that('RMSE should be on the y-axis.', {
        expect_true("RMSE" %in% c(rlang::get_expr(properties$y), rlang::get_expr(properties$y)))
    })
...
test_10.0 <- function(){
    ...
    test_that('values of predictions are incorrect', {
        expect_equal(digest(round(as.numeric(mean(predictions)))), 'dbc09cba9fe2583fb01d63c70e1555a8')
    })
    print("Success!")
}
...
test_13.0 <- function(){
    properties <- c(predict_plot$mapping, predict_plot$layers[[1]]$mapping)
    labels <- answer2$labels
    test_that('Did not create a plot named predict_plot', {
        expect_true(exists("predict_plot")) 
    })
    test_that('max should be on the x-axis.', {
        expect_true("max" %in% c(rlang::get_expr(properties$x),rlang::get_expr(properties$x)))
    })
    test_that('time_hrs should be on the y-axis.', {
        expect_true("time_hrs" %in% c(rlang::get_expr(properties$y), rlang::get_expr(properties$mapping$y)))
    })
