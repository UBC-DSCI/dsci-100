...
test_that('as.factor() function not used.',{
      expect_equal(c(levels(fruit_data$fruit_name)), c("apple", "lemon", "mandarin", "orange" ))
})
print("Success!")
    }
...
test_that('Accuracy should be on the y-axis.', {
    expect_true("Accuracy" == rlang::get_expr(properties$y))
    })
...
test_3.3 <- function(){
test_that('Solution is incorrect', {
    expect_that(exists('answer3.3'), is_true())
    expect_equal(digest(answer3.3), 'c1f86f7430df7ddb256980ea6a3b57a4') # we hid the answer to the test here so you can't see it, but we can still run the test
})
print("Success!")
    }
...
test_3.4 <- function(){
test_that('Solution is incorrect', {
    expect_true(answer3.4 > 0.92)
    expect_true(answer3.4 < 0.94)
})
print("Success!")
    }
...
test_that('X_complex does not contain the correct columns.', {
    expect_true("height" %in% colnames(X_complex))
    expect_true("width" %in% colnames(X_complex))
    expect_true("mass" %in% colnames(X_complex))
    expect_true("color_score" %in% colnames(X_complex))
})
...
test_that('k should be on the x-axis.', {
    expect_true("k" %in% c(rlang::get_expr(choose_k_again_plot$mapping$x),rlang::get_expr(choose_k_again_plot$layers[[1]]$mapping$x)))
    })
test_that('Accuracy should be on the y-axis.', {
    expect_true("Accuracy" %in% c(rlang::get_expr(choose_k_again_plot$mapping$y), rlang::get_expr(choose_k_again_plot$layers[[1]]$mapping$y)))
    })
