...
test_1.8 <- function(){
    test_that('beer_ks should be a data frame.', {
    expect_true('data.frame' %in% class(beer_ks))
    })
test_that('beer_ks should have 1 column containing k values from 1 to 10.', {
    expect_equal(nrow(beer_ks), 10) 
    expect_equal(ncol(beer_ks), 1) 
...
test_that('The model_statistics column in beer_model_stats should be a data frame.', {
    expect_true('data.frame' %in% class(beer_model_stats$model_statistics[[1]]))
    })
print("Success!")
    }
